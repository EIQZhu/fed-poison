...
class Config:
    ...
    def run_mp(self):
        ...
        cluster_conf = {
            ...
            "neighborhood_size": self.config["neighborhood_size"],  # 全局配置项
            "client": [
                {
                    "ip": self.config["client" + str(i)]["ip"],
                    "port": self.config["client" + str(i)]["port"],
                    "idx": self.config["client" + str(i)]["idx"]
                }
    ...
    def run_distributed(self):
        ...
        if self.config["role"] == "server":
            ...
            for i in range(self.config["n_clients"]):
                torch.save(data.trainLoader[i].dataset, f"./datasets/{self.config['data']}_{i + 1}")
            ...
            data = torch.load(f"./datasets/{self.config['data']}_{self.config['idx']}")
            loader = DataLoader(
                dataset=data,
                batch_size=self.config["batch_size"],
                shuffle=True,
