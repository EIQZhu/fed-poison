...
class TrainerMP:
    def __init__(
        ...
        self.neighborhood_size = self.cluster_conf['neighborhood_size']
        ...
        self.clients = [all_client[algorithm](
            self.clients_ip_port[i]["idx"],
            self.clients_ip_port[i]["ip"],
            self.clients_ip_port[i]["port"],
            #self.clients_ip_port[i]["idx"],
            self.server_ip,
            self.server_port,
            self.model,
        ...
        self.clients_process = [mp.Process(
            ...
            name=f"CLIENT@{self.clients_ip_port[i]['ip']}:{self.clients_ip_port[i]['ip']}")
            for i in range(self.n_clients)]
