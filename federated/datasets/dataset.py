...
class Data:
    def __init__(self, plot_path):
        self.train_set = None
        self.validxate_set = None
        self.plot_path = plot_path

    def train_loader(self, alpha, n_clients, batch_size, flag=False):
        labels = np.array(self.train_set.targets)
        split_idx = self.split_non_iidx(labels, alpha=alpha, n_clients=n_clients)
        if flag:
            plt.figure(figsize=(20, 3))
            plt.hist([labels[idxc] for idxc in split_idx], stacked=True,
                     bins=np.arange(min(labels) - 0.5, max(labels) + 1.5, 1),
                     label=["Client {}".format(i + 1) for i in range(n_clients)], rwidxth=0.5)
    ...
    def validxate_loader(self, batch_size):
        return DataLoader(self.validxate_set, batch_size=batch_size, shuffle=False)

    @staticmethod
    def split_non_iidx(train_labels, alpha, n_clients):
