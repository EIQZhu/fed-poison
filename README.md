...
以FedAVG在Iidx情况下，使用一个简单的CNN网络训练MNIST为例
...
- 主进程开多个子进程版本
  ...
  该文件配置了Iidx情况下，将MNIST的训练集分到十个客户端进行联邦学习（测试集放在server中），使用了一个简单的CNN网络，
...
成员变量train_set为训练集，validxate_set为验证集（测试集）。成员函数train_loader传入alpha（狄利克雷参数），n_clients（客户端数量），batch_size(切割的每个数据集的batch大小)，flag（是否绘制数据分布图像）最后返回一个包括n_clients数量的DataLoader列表（torch.utils.data.DataLoader），每个DataLoader的数据量以及总数据量。
静态方法split_non_iidx使用numpy的狄利克雷分布对数据集进行切分。

添加的新数据可参考mnist.py，继承Data后调用父类构造函数，需要指定train_set和validxate_set（类型为torch.utils.data.Dataset），接着调用父类的train_loader成员函数和validxate_loader成员函数得到相应内容。
...
成员函数first_push为训练前发布服务器的参数，pull为拉取所有client的参数，aggregate为聚合所有的参数，push发布自身参数，validxate验证聚合后的参数性能，server的行为被封装进了run函数中。\
